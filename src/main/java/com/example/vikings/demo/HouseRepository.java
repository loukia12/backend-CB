...
public interface HouseRepository extends JpaRepository<House, Integer> {
	...
	@Query("SELECT h FROM House h WHERE (h.price = ?1 or h.price < ?1) and (h.size = ?2 or h.size > ?2) and (h.location = ?3)")
	List<House> test(Integer price, Integer size, String location);
...
//	
//	@Query("SELECT h FROM House h WHERE (h.price = ?1 or h.price < ?1) and (h.size = ?2 or h.size > ?2) ORDER BY h.price ASC")
//	List<House> findByFilters3(Integer price, Integer size);
}
